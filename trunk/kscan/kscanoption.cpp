...
inline const SANE_Option_Descriptor *getOptionDesc( const QByteArray& name )
{
   ...
   {
      qDebug() << "no option descriptor for <" << name << ">";
      // debug( "Name survived !" );
   }
   ...
   QObject()
{
   ...
   {
      qDebug() << "Had problems to create KScanOption - initOption failed !";
   }
}
...
bool KScanOption::initOption( const QByteArray& new_name )
{
   ...
   if( desc )
   {
	...
	{
	   // qDebug() << "Is NOT older GammaTable!";
	}
    }
   ...
   QObject()
{
   ...
   {
      qWarning() << "Trying to copy a not healthy option (no name nor desc)";
      return;
   }
   
   if( so.buffer_untouched )
      qDebug() << "Buffer of source is untouched!";
   ...
   switch( desc->type )
   {
       ...
       break;
       default:
	  qWarning() << "unknown option type in copy constructor";
	  break;
    }
}
...
const QString KScanOption::configLine( void )
{
   ...
   qDebug() << "configLine returns <" << strval << ">";
   return( strval );
}
...
{
    qDebug() << "Received WidgetChange for " << getName() << " (const QCString&)";
    set( t );
    emit( guiChange( this ) );
...
{
    qDebug() << "Received WidgetChange for " << getName() << " (int)";
    set( i );
    emit( guiChange( this ) );
    // emit( optionChanged( this ));
}
...
void KScanOption::slReload( void )
{
   ...
   if( widget() )
   {
      qDebug() << "constraint is " << desc->cap;
      if( !active() )
	 qDebug() << desc->name << " is not active now";

      if( !softwareSetable() )
	 qDebug() << desc->name << " is not software setable";

      if( !active() || !softwareSetable() )
      {
	 qDebug() << "Disabling widget " << getName() << " !";
   ...
   if( active())
   {
	 ...
	 if( sane_stat != SANE_STATUS_GOOD )
	 {
	    qDebug() << "ERROR: Cant get value for " << getName() << ": " << sane_strstatus( sane_stat );
	 } else {
	    buffer_untouched = false;
	    qDebug() << "Setting buffer untouched to FALSE";
	 }
      }
   }
}
...
bool KScanOption::set( int val )
{
   ...
   switch( desc->type )
   {
	 ...
	 break;
       default:
	 qDebug() << "Cant set " << name << "  with type int";
   }
   if( ret )
   {
      buffer_untouched = false;
...
bool KScanOption::set( double val )
{
   ...
   switch( desc->type )
   {
	 ...
	 break;
      default:
	 qDebug() << "Cant set " << name << " with type double";
   }
...
bool KScanOption::set( int *val, int size )
{
	...
	qDebug() << "Size: " << size << ", word_size: " << word_size << ", descr-size: "<< desc->size;
	qa[0] = (SANE_Word) 1+size;
	qDebug() << "set length field to " << qa[0] <<endl;
	offset = 1;
    }
#endif
	...
	qDebug() << "Copying " << copybyte << " byte to options buffer";
...
bool KScanOption::set( const QByteArray& c_string )
{
   ...
   if( QString(c_string).contains( re ))
   {
      ...
      ret = set( &gt );
      qDebug() << "Setting GammaTable with int vals " << brig << "|" << contr << "|" << gamm;
   ...
   {
       case SANE_TYPE_STRING:
	   qDebug() << "Setting " << c_string << " as String";
	   ...
	   {
	       qDebug() << "ERROR: Buffer for String " << c_string << " too small: " << buffer_size << "  < " << c_string.length();
	   }
	   break;
       case SANE_TYPE_INT:
       case SANE_TYPE_FIXED:
	   qDebug() << "Type is INT or FIXED, try to set value <" << c_string << ">";
	   val = c_string.toInt( &ret );
	   if( ret ) 
	       set( &val, 1 );
	   else
	       qDebug() << "Conversion of string value failed!";
	   
	   break;
      case SANE_TYPE_BOOL:
	 qDebug() << "Type is BOOL, setting value <" << c_string << ">";
	 val = 0;
	 if( c_string == "true" ) val = 1;
	 set( val );
	 break;
       default:
	   qDebug() << "Type of " << name << " is " << desc->type;
	   qDebug() << "Cant set " << name << " with type string";
	   break;
   }
...
bool KScanOption::set( KGammaTable *gt )
{
   ...
   qDebug() << "KScanOption::set for Gammatable !";
   switch( desc->type )
   {
      case SANE_TYPE_INT:
	 for( int i = 0; i < word_size; i++ ){
	 ...
	 break;
      default:
	 qDebug() << "Cant set " << name << " with type GammaTable";
	 ret = false;
	...
	*val = 0;
    ...
    case SANE_TYPE_FIXED:
      ...
      *val = (int)d;
      break;
    default:
      qDebug() << "Cant get " << name << " to type int";
      return( false );
    }
...
QByteArray KScanOption::get( void ) const
{
   ...
   switch( desc->type )
   {
	 ...
	 break;
      ...
      default:
	 qDebug() << "Cant get " << getName() << " to type String !";
	 retstr = "unknown";
   }
   ...
   if( type() == GAMMA_TABLE )
   {
      retstr = QString().sprintf( "%d, %d, %d", gamma, brightness, contrast );
   }

   qDebug() << "option::get returns " << retstr;
   return( retstr );
}
...
bool KScanOption::getRangeFromList( double *min, double *max, double *q ) const
{
   ...
   if ( desc->constraint_type == SANE_CONSTRAINT_WORD_LIST ) {
	    ...
	    qDebug() << "Resolutions are in a word list";
            const SANE_Int *sint =  desc->constraint.word_list;
            int amount_vals = *sint; sint ++;
	    double value;
	    *min = 0;
      ...
      qDebug() << "getRangeFromList: No list type " << desc->name;
      ret = false;
    }
    return( ret );
}
...
bool KScanOption::getRange( double *min, double *max, double *q ) const
{
   ...
   if( desc->constraint_type == SANE_CONSTRAINT_RANGE ||
	  ...
	  *q   = r->quant ;
      ...
      qDebug() << "getRange: No range type " << desc->name;
      ret = false;
   }
   return( ret );
}
                                    ...
                                    const QString& tooltip )
{
  ...
  if( ! valid() )
  {
     qDebug() << "The option is not valid!";
     return( 0 );
  }
  QWidget *w = 0;
  /* free the old widget */
  ...
  switch( type( ) )
    ...
    case SINGLE_VAL:
      ...
      w = 0; // new QEntryField(
      qDebug() << "can not create widget for SINGLE_VAL!";
      break;
    case RANGE:
      /* Widget Type is Slider */
    ...
    case GAMMA_TABLE:
      /* Widget Type is Slider */
      // w = KSaneSlider( parent, text );
      qDebug() << "can not create widget for GAMMA_TABLE!";
    ...
    case STRING:
      ...
      break;
    default:
       qDebug() << "returning zero for default widget creation!";
      w  = 0;
      break;
    }
...
bool KScanOption::applyVal( void )
{
   ...
   {
      qDebug() << "Error in in situ appliance " << getName() << ": " << sane_strstatus( stat );
      res = false;
   }
   else
   {
      qDebug() << "IN SITU appliance " << getName() << ": OK";
